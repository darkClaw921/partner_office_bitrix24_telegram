...
- `main.py` — основной файл с FastAPI приложением. Содержит:
  ...
  - Функция обновления поля PARTNER_URLS в сделке (`_update_deal_partner_url`)
...
- Пропуск обработки, если ID равен `"0"` или пустой строке (в Bitrix24 `"0"` означает отсутствие привязки)

### 4. Обновление поля PARTNER_URLS в сделке
- После успешной генерации кода партнера обновление поля `PARTNER_URLS` в сделке
- Формирование URL по шаблону: `https://auto-legal.ru/promo?utm_term={partner_code}`
- Приоритет отдается коду контакта, если обработаны и контакт, и компания
...
- Информация об успешности обновления поля `PARTNER_URLS`
